...
```json
{
  "storage_root": "/Users/<your-user>/.local/share/giteam/remote-repo-service"
}
```
...
如果不想每次传 `REMOTE_REPO_SERVICE_URL`，可以在 Giteam 的设置页中保存远程仓库服务地址。服务端启用 API key 时，也在同一处保存 key。
