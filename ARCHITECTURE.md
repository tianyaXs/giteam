...
**Giteam** 是一款面向开发者的**多端 Git 协作工具**，支持在桌面端、移动端和 Web 端访问同一个本地 Git 代码仓库，并内置 AI 辅助编程能力（Host 进程内嵌 Pi Agent / giteam-core）。
...
> **Host 模型（现行）**：Desktop 与 CLI 均为基于 `giteam-core` 的平级 Host；Desktop **进程内嵌** Control，不再依赖 spawn 外部 CLI 起服。偏好端口默认 4100，占用则顺延；手机扫码使用实际 `listeningPort` / LAN URL。详情见 `docs/ARCHITECTURE_SUMMARY.md`。
...
│  │  ┌──────────┐ ┌──────────┐ ┌──────────┐           │    │
│  │  │ control  │ │ pi_agent │ │ desktop_rpc│          │    │
│  │  │(HTTP Host)│ │(AI 运行时)│ │(桌面RPC) │           │    │
│  │  └──────────┘ └──────────┘ └──────────┘           │    │
│  └────────────────────────────────────────────────────┘    │
│  ┌────────────────────────────────────────────────────┐    │
│  │  Desktop Host：进程内嵌 Control（与 CLI 平级）       │    │
│  │  CLI Host：`giteam service` 内嵌同一套 Control      │    │
│  │  二者可同时运行；偏好端口占用时自动顺延              │    │
│  └────────────────────────────────────────────────────┘    │
└────────────────────────────────────────────────────────────┘
          │
...
│                      数据与运行时层                           │
│  ┌──────────────┐  ┌──────────────┐  ┌──────────────────┐  │
│  │  SQLite DB   │  │  Git CLI     │  │  Pi Agent (进程内) │  │
│  │ (client.db)  │  │  (git)       │  │  + 模型凭据/可见性 │  │
│  └──────────────┘  └──────────────┘  └──────────────────┘  │
└─────────────────────────────────────────────────────────────┘
```
