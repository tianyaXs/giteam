...
fn run_entire(args: &[&str], repo_path: &str) -> Result<String, String> {
    command_runner::run_and_capture_in_dir_with_timeout("entire", args, repo_path, EXPLAIN_TIMEOUT_SECS)
}
...
pub fn run_entire_explain_commit(commit_sha: &str, repo_path: &str) -> Result<String, String> {
    ...
    run_entire(&["explain", "--commit", commit_sha, "--no-pager"], repo_path)
}

#[tauri::command]
pub fn run_entire_explain_commit_short(commit_sha: &str, repo_path: &str) -> Result<String, String> {
    command_runner::validate_commit_sha(commit_sha)?;
    run_entire(&["explain", "--commit", commit_sha, "--short", "--no-pager"], repo_path)
}

#[tauri::command]
pub fn run_entire_explain_checkpoint(checkpoint_id: &str, repo_path: &str) -> Result<String, String> {
