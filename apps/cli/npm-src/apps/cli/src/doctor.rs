...
use giteam_core::{command_runner, control};
use serde::Serialize;
use std::env;
...
pub fn build_report(repo_path: Option<String>, warmup: bool) -> Result<DoctorReport, String> {
    ...
    let binaries = vec![
        ...
        detect_binary_status(
            "node",
            &["--version"],
            "install Node.js for skills CLI installs",
        ),
    ];
    ...
    if warmup_status.attempted && !warmup_status.ready {
        failures.push(
            warmup_status
                .error
                .clone()
                .unwrap_or_else(|| "pi agent warmup failed".to_string()),
        );
    }
    ...
    Ok(DoctorReport {
        ...
        agent: DoctorAgent {
            ...
            ready: warmup_status.ready || !warmup,
...
pub fn render_human(report: &DoctorReport) -> String {
    ...
    lines.push(format!("  runtime: {}", report.agent.runtime));
    lines.push(format!(
        "  ready: {}",
        if report.agent.ready { "yes" } else { "no" }
    ));
    if report.agent.warmup.attempted {
        lines.push(format!(
            "  warmup: {}{}{}",
            if report.agent.warmup.ready {
                "ready"
            } else {
                "failed"
            },
...
fn build_warmup_status(repo_path: Option<&str>, warmup: bool) -> WarmupStatus {
    ...
    let info = PiAgentService::global().runtime_info();
    WarmupStatus {
        attempted: true,
        ready: true,
        base_url: Some(format!("pi://{}", info.backend)),
        error: None,
    }
}
